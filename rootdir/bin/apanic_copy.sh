...
if [ -z "$ignore_apanic_logs" ]; then
    ...
    kpgather
    # read minidump to dontpanic
    if [ -e /vendor/bin/ramdump_dumper ] ; then
        ramdump_dumper save minidump /data/vendor/dontpanic/minidump
        if [ -e /data/vendor/dontpanic/minidump.tar.gz ] ; then
            chmod 0640 /data/vendor/dontpanic/minidump.tar.gz
        fi
    fi
else
